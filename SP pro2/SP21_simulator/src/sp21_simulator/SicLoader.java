	...
	public void load(File objectCode) throws IOException {
		...
		while((line=bufread.readLine())!=null) {
			if(line.charAt(0)=='D')
			{
				...
				{
					rMgr.symtabList.putSymbol(line.substring(i,i+6), rMgr.proglength+Integer.parseInt(line.substring(i+6,i+12),16));
				}
			}
		...
		while((line=bufread2.readLine())!=null) {
			if(line.charAt(0)=='H')//Header
			{
				if(count==0)//첫 프로그램의 경우
				{
					...
					rMgr.sttadd=sttadd;
					length=Integer.parseInt(line.substring(13),16);
					rMgr.symtabList.putSymbol(line.substring(1,7), sttadd);
					count+=1;
				}
				else
				{
					rMgr.proglength+=length;
					length=Integer.parseInt(line.substring(13),16);
					rMgr.symtabList.putSymbol(line.substring(1,7),rMgr.proglength );
				}
			}
			else if(line.charAt(0)=='M')
			{
				float fixadd=rMgr.proglength+(float)Integer.parseInt(line.substring(1,7),16);
			...
			{
				int stad=Integer.parseInt(line.substring(1,7),16);
				int lenofline=0;
